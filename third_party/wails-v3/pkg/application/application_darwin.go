...
static NSWindow* currentApplicationWindow(void) {
...
static void enableNativeFullscreen(NSWindow *window) {
	if (window == nil) {
		return;
	}
...
static void init(void) {
	...
	[NSEvent addLocalMonitorForEventsMatchingMask:NSEventMaskKeyDown handler:^NSEvent * _Nullable(NSEvent * _Nonnull event) {
		...
		if ((modifierFlags & NSEventModifierFlagControl) &&
			...
			[event keyCode] == 3) {
			NSWindow *window = currentApplicationWindow();
			if (window != nil) {
...
static void activateThenApplyPolicy(int policy) {
...
func (m *macosApp) run() error {
	...
	m.parent.On(events.Mac.ApplicationDidFinishLaunching, func(*Event) {
		...
		C.activateThenApplyPolicy(C.int(m.parent.options.Mac.ActivationPolicy))
	})
	m.setupCommonEvents()
